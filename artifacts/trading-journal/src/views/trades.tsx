...
export default function Trades() {
  ...
  const { data: trades, isLoading } = useListTrades(
    { assetType: assetTypeFilter !== 'all' ? assetTypeFilter as any : undefined },
    { query: { queryKey: getListTradesQueryKey({ assetType: assetTypeFilter !== 'all' ? assetTypeFilter as any : undefined }) } }
  )
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {isLoading ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-muted-foreground">Loading trades...</td>
                  </tr>
                ) : trades?.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-8 text-center text-muted-foreground">No trades found.</td>
                  </tr>
                ) : (
                  trades?.map((trade) => (
  ...
  return (
    <tr className="border-b border-border hover:bg-muted/30 transition-colors">
      ...
      <td className="px-4 py-3">
        <span className={`px-2 py-0.5 rounded-sm text-xs font-bold uppercase ${trade.direction === 'long' ? 'bg-primary/20 text-primary' : 'bg-destructive/20 text-destructive'}`}>
          {trade.direction}
        </span>
      </td>
      <td className="px-4 py-3 text-right tabular-nums">${Number(trade.entryPrice).toFixed(2)}</td>
      <td className="px-4 py-3 text-right tabular-nums">
        {trade.exitPrice ? `$${Number(trade.exitPrice).toFixed(2)}` : <span className="text-muted-foreground">-</span>}
      </td>
      <td className="px-4 py-3 text-right tabular-nums">{trade.quantity}</td>
      <td className={`px-4 py-3 text-right font-bold tabular-nums ${trade.pnl != null ? (trade.pnl >= 0 ? 'text-primary' : 'text-destructive') : ''}`}>
        ...
        <div className="flex justify-end gap-2">
          <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <TradeForm initialData={trade} onSuccess={() => setIsEditOpen(false)} />
            </DialogContent>
          </Dialog>
          <Button variant="ghost" size="sm" className="h-8 px-2 text-xs text-destructive hover:text-destructive" onClick={handleDelete}>
            Del
          </Button>
        </div>
      </td>
